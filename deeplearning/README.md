...
下面,我们进行一个大胆的尝试:保留图片中的二维信息.一个简单的做法是,把图片拆分成4个区域(就是上图中的红色,绿色,黄色,蓝色四个区域).每个感知器仅仅处理一个区域的内容.
...
这样一来,每个节点都能够提取图像中的某些特征.你会问:这就是卷积层吗?很遗憾,这并不是卷积层,但是我们已经很接近了!
...
我们还是把图片拆分为多个部分,这些部分的尺寸应该是一样的.对于二维图片来说,这里定义一个"卷积窗".卷积窗拥有width和height两个属性,描述了窗口的size.

另外,卷积窗的每个节点都有保存了一个参数.

随后,将卷积窗在图片上水平垂直地滑动,每次滑动,都对卷积窗包含的像素点使用卷积窗的参数进行一次计算.这样,卷积窗滑动了多少次,就计算出了多少个结果.
