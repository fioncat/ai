...
如果$W_s$很大,那么多个$W_s$累乘在一起就会产生一个很大的结果,这个时候算得的梯度就会非常大,这叫做梯度膨胀.

在训练RNN的时候,梯度消失和梯度膨胀都有可能出现.解决梯度膨胀比较简单,我们可以设置一个阈值,当梯度超过这个阈值之后,就对梯度进行正则化操作使其保持在一个比较小的范围内.

但是梯度消失就没有这么好解决了,你可能想的是,让RNN的层数不要这么多,可是这样就损失了RNN的强壮性,另外,当t很大时,还是会出现梯度消失.

另外一个可能行得通的方法是使用导数更大的激活函数,但是偏导数过深仍然存在梯度消失的隐患.
...
RNN在处理信息的时候,只是简单的使用权值去计算然后用激活函数产生一个新的信息,而在LSTM内部,则会有很多门来控制LSTM单元遗忘,学习,记忆,应用信息.并且所有信息都分为LTM和STM.一个LSTM单元内部的结构如下:
